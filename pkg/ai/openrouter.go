...
func AnalyzeHighlights(entries []transcriber.SubtitleEntry, apiKey, model, targetLang string) ([]AIHighlight, error) {
	...
	highlights, err := callOpenAICompatibleAPI(openRouterEndpoint, entries, resolvedKey, resolvedModel, targetLang)
	if err != nil && resolvedModel != "openrouter/free" {
		fmt.Printf("[AI WARN] Model '%s' failed: %v. Retrying with fallback model 'openrouter/free'...\n", resolvedModel, err)
		return callOpenAICompatibleAPI(openRouterEndpoint, entries, resolvedKey, "openrouter/free", targetLang)
	}
